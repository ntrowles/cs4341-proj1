...
## Documentation ##
This program is meant to solve the problem presented in assignment 2 for CS4313 at Worcester
Polytechnic Institute. For this problem, the program had to read in a search problem, and then
execute a genetic algorithm search on the given problem.
...
## How it Works ##
*Note* This document references the 
...
either a GreedySearch, IterativeDeepeningSearch, or GeneticAlgorithmSearch, and then call that object's search funtion(),
...
object. SolutionInfo contains all required information about the solution:
	...
	* Nodes Expanded during the search/Size of the population (for Genetic Search)
	* Maximum search depth of the search/Number of generations (for Genetic Search
	* Error number:
...
The GeneticAlgorithmSearch search() method behaves differently than both the greedy and iterative deepening
searches. It starts by generating an initial population, and testing if any of them are a valid answer. If
it finds a valid solution, it returns that. If there isn't a valid solution in the initial population, it will
keep creating new generations until time runs out, or until it finds a valid solution.

To produce a new generation, elitism and culling are first applied to move good solutions to the next generation,
and to remove bad solutions from the population, respectively. After doing so, the program will select 2 parents
from the current generation based on their fitness, and generate a new child. There is a chance that this child
will be mutated, and have an element inserted into, removed from, or altered in its path. This occurs until the new
population is the same size as the current population, and then the new population becomes the current population.